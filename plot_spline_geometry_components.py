...
def plot(folder,out):
    ...
    values=[];checks=[]
    for name in ('gt','unregularized','intrinsic001'):
        p=arrays[name];camera=decompose_physical_camera(p)
    ...
    legends=['GT spline','Estimated: no prior','Estimated: intrinsic lambda=0.01']
    colors=['black','#2469b2','#8a45b7']
    out.mkdir(parents=True,exist_ok=True)
    fig,axes=plt.subplots(3,3,figsize=(15,10),sharex=True,layout='constrained')
    for j,ax in enumerate(axes.flat):
        for i in (1,2,0):
            ax.plot(theta,values[i][:,j],color=colors[i],ls='--' if i==0 else '-',
                    lw=1.3 if i==0 else 1.,label=legends[i])
    ...
    fig.legend([handles[i] for i in (2,0,1)],[text[i] for i in (2,0,1)],loc='outside lower center',ncol=3)
    ...
    np.savez(out/'geometry_components9.npz',theta_deg=theta,gt=values[0],unregularized=values[1],intrinsic001=values[2])
    with (out/'geometry_components9.csv').open('w',newline='') as stream:
        writer=csv.writer(stream,lineterminator='\n')
        writer.writerow(['view','theta_deg']+[f'{run}_{p}' for run in ('gt','unregularized','intrinsic001') for p in NAMES])
